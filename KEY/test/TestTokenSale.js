...
contract('TokenSale', async (accounts) => {
  ...
  it('Reject purchases under 0.5 eth', async function () {
    ...
    await tryCatch(tokenSale.buyTokens({value: web3.utils.toWei('0.3', 'ether'), from: accounts[1]}), errTypes.revert);
  })

  it('Purchase 2000 Tokens with 1 Ether', async function () {
    await tokenSale.addToWhitelist(accounts[5], {from: accounts[0]});
    // Purchase tokens
    await tokenSale.buyTokens({value: web3.utils.toWei('1', 'ether'), from: accounts[5]});
  ...
  it('Pause and unpause sale', async function() {
    ...
    await tryCatch(tokenSale.buyTokens({value: web3.utils.toWei('1', 'ether'), from: accounts[5]}), errTypes.revert);
  ...
  it('Tiers switch automatically when limits are reached', async function () {
    ...
    await tokenSale.buyTokens({value: web3.utils.toWei(web3.utils.toBN(maxEthTiers[0]), 'ether'), from: accounts[5]});

    // Purchase up to the limit on 2nd tier, then try to purchase more
    await tokenSale.buyTokens({value: web3.utils.toWei(web3.utils.toBN(maxEthTiers[1]), 'ether'), from: accounts[5]});
  ...
  //   await tokenSale.buyTokens({value: web3.utils.toWei(web3.utils.toBN(maxEthTiers[0] - 1), 'ether'), from: accounts[5]});
  //   switchTimes[0] = web3.eth.getBlock("latest").timestamp;
  //   await tokenSale.buyTokens({value: web3.utils.toWei(web3.utils.toBN(2), 'ether'), from: accounts[5]});
  //
  //   // Purchase up to the limit on 2nd tier, then try to purchase more
  //   await tokenSale.buyTokens({value: web3.utils.toWei(web3.utils.toBN(maxEthTiers[1] - 3), 'ether'), from: accounts[5]});
  //   switchTimes[1] = web3.eth.getBlock("latest").timestamp;
  //   console.log(switchTimes[0]);
  //   await tokenSale.buyTokens({value: web3.utils.toWei(web3.utils.toBN(2), 'ether'), from: accounts[5]});
  //
  //   // Block timestamps should be equal
  //   assert.equal(await tokenSale.getStageSwitchTimestamp.call(0), web3.utils.toBN(switchTimes[0]));
  //   assert.equal(await tokenSale.getStageSwitchTimestamp.call(1), web3.utils.toBN(switchTimes[1]));
  //
  // })
  ...
  it('Unsold Tokens return to contract owner', async function () {
    ...
    await tokenSale.buyTokens({value: web3.utils.toWei('40', 'ether'), from: accounts[7]});
    await tokenSale.buyTokens({value: web3.utils.toWei('40', 'ether'), from: accounts[8]});


    // Should purchase 50000 tokens into one account
    await tokenSale.buyTokens({value: web3.utils.toWei('20', 'ether'), from: accounts[7]});


    // Should purchase 50000 tokens into one account
    await tokenSale.buyTokens({value: web3.utils.toWei('20', 'ether'), from: accounts[8]});

    // Should purchase 200000 tokens into one account
    await tokenSale.buyTokens({value: web3.utils.toWei('80', 'ether'), from: accounts[6]});
  ...
  it('Tokens should burn when burn function called', async function () {
    ...
    await tokenSale.addToWhitelist(accounts[6], {from: accounts[0]});
    await tokenSale.buyTokens({value: web3.utils.toWei('10', 'ether'), from: accounts[6]});
